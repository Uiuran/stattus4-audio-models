...
def amostragem(data, fs, plot = False):
  ...
  f,t,Sxx = signal.spectrogram(data,fs,nfft=fs/4,nperseg=fs/5,noverlap=fs/10,scaling='spectrum')
...
####
## Downsampling according to Nyquist-Shannon
#
sampa = np.max(np.array(amostragem(data1,fs1))[:,1])
sampb = np.max(np.array(amostragem(data,fs))[:,1])
fsa = (fs1/int(sampa))
fsb=(fs/int(sampb))

a = downsampling(data1,sampa )
b = downsampling(data,sampb )
